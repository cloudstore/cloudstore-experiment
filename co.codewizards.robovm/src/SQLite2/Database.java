...
public class Database {
    ...
    public void open(String filename, int mode) throws SQLite2.SQLiteException {
	if ((mode & 0200) != 0) {
	    mode = SQLite2.Constants.SQLITE_OPEN_READWRITE |
		   SQLite2.Constants.SQLITE_OPEN_CREATE;
	} else if ((mode & 0400) != 0) {
	    mode = SQLite2.Constants.SQLITE_OPEN_READONLY;
	}
	synchronized(this) {
	    try {
		_open4(filename, mode, null, false);
	    } catch (SQLite2.SQLiteException se) {
		throw se;
	...
	throws SQLite2.SQLiteException {
	if ((mode & 0200) != 0) {
	    mode = SQLite2.Constants.SQLITE_OPEN_READWRITE |
		   SQLite2.Constants.SQLITE_OPEN_CREATE;
	} else if ((mode & 0400) != 0) {
    ...
    /*
     * For backward compatibility to older SQLite2.jar, sqlite_jni
     */
    ...
    public void open_aux_file(String filename) throws SQLite2.SQLiteException {
	synchronized(this) {
    ...
    public void close()	throws SQLite2.SQLiteException {
	synchronized(this) {
	    _close();
	}
    }
     ...
     *
     * It the method fails, an SQLite2.Exception is thrown and
    ...
    public void exec(String sql, SQLite2.Callback cb) throws SQLite2.SQLiteException {
	synchronized(this) {
	    _exec(sql, cb);
	}
    }

    private native void _exec(String sql, SQLite2.Callback cb)
	throws SQLite2.SQLiteException;
    ...
    public void exec(String sql, SQLite2.Callback cb,
		     String args[]) throws SQLite2.SQLiteException {
	synchronized(this) {
	    _exec(sql, cb, args);
	}
    }

    private native void _exec(String sql, SQLite2.Callback cb, String args[])
	throws SQLite2.SQLiteException;
    ...
    public void busy_handler(SQLite2.BusyHandler bh) {
	synchronized(this) {
	    _busy_handler(bh);
	}
    }

    private native void _busy_handler(SQLite2.BusyHandler bh);
    ...
    public TableResult get_table(String sql) throws SQLite2.SQLiteException {
     ...
     * @param type return type code, e.g. SQLite2.Constants.SQLITE_NUMERIC
     */
    ...
    public void set_encoding(String enc) throws SQLite2.SQLiteException {
	synchronized(this) {
	    _set_encoding(enc);
	}
    }
    ...
    public Vm compile(String sql) throws SQLite2.SQLiteException {
	synchronized(this) {
	    Vm vm = new Vm();
	    vm_compile(sql, vm);
	    return vm;
    ...
    public Vm compile(String sql, String args[]) throws SQLite2.SQLiteException {
	synchronized(this) {
	    Vm vm = new Vm();
    ...
    public Stmt prepare(String sql) throws SQLite2.SQLiteException {
	synchronized(this) {
	    Stmt stmt = new Stmt();
    ...
    public Blob open_blob(String db, String table, String column,
			  long row, boolean rw) throws SQLite2.SQLiteException {
	synchronized(this) {
	    Blob blob = new Blob();
    ...
    public void progress_handler(int n, SQLite2.ProgressHandler p) {
	synchronized(this) {
	    _progress_handler(n, p);
	}
    }

    private native void _progress_handler(int n, SQLite2.ProgressHandler p);

    /**
     * Specify key for encrypted database. To be called
     * right after open() on SQLite3 databases.
     * Not available in public releases of SQLite2.
     *
     * @param ekey the key as byte array
     */

    public void key(byte[] ekey) throws SQLite2.SQLiteException {
	synchronized(this) {
	    _key(ekey);
	}
    }
    ...
    public void key(String skey) throws SQLite2.SQLiteException {
	synchronized(this) {
	    byte ekey[] = null;
    ...
    public void rekey(byte[] ekey) throws SQLite2.SQLiteException {
	synchronized(this) {
	    _rekey(ekey);
	}
    }
    ...
    public void rekey(String skey) throws SQLite2.SQLiteException {
	synchronized(this) {
	    byte ekey[] = null;
	    if (skey != null && skey.length() > 0) {
		ekey = new byte[skey.length()];
    ...
    public static long long_from_julian(String s) throws SQLite2.SQLiteException {
	try {
	    double d = Double.parseDouble(s); // android-changed: performance
	    return long_from_julian(d);
	} catch (java.lang.Exception ee) {
	    throw new SQLite2.SQLiteException("not a julian date");
	}
    }
    ...
    static {
	try {
	    String path = System.getProperty("SQLite2.library.path");
